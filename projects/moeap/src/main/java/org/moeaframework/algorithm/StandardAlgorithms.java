...
import static org.moeaframework.core.comparator.PopulationIndependentDominanceComparatorProvider.CROWDING_PROVIDER;
import static org.moeaframework.core.comparator.PopulationIndependentDominanceComparatorProvider.PARETO_PROVIDER;
...
import org.moeaframework.core.comparator.ChainedComparatorProvider;
...
public class StandardAlgorithms extends AlgorithmProvider {
	...
	private Algorithm neweMOEA(TypedProperties properties, Problem problem) {
		...
		final TournamentSelection selection = new TournamentSelection(2, PARETO_PROVIDER);
		...
		EpsilonMOEA emoea = new EpsilonMOEA(problem, population, archive,
				selection, variation, initialization, PARETO_PROVIDER);
	...
	private Algorithm newNSGAII(TypedProperties properties, Problem problem) {
		...
		DominanceComparatorProvider dominanceComparatorProvider;
		DominanceComparatorProvider preferenceComparatorProvider = problem.getPreferenceComparatorProvider();
		if(preferenceComparatorProvider == null) {
			dominanceComparatorProvider = PARETO_PROVIDER;
		} else {
			dominanceComparatorProvider = new ChainedComparatorProvider(PARETO_PROVIDER, preferenceComparatorProvider);
		}
		
		
		NondominatedSortingPopulation population = 
				new NondominatedSortingPopulation(dominanceComparatorProvider);

		TournamentSelection selection = new TournamentSelection(2, 
				new ChainedComparatorProvider(
						dominanceComparatorProvider,
						CROWDING_PROVIDER));

		Variation variation = OperatorFactory.getInstance().getVariation(null, 
				properties, problem);

		NSGAII algorithm = new NSGAII(problem, population, null, selection, variation, initialization);
		return algorithm;
	}
	...
	private Algorithm newGDE3(TypedProperties properties, Problem problem) {
		...
		NondominatedSortingPopulation population = 
				new NondominatedSortingPopulation(PARETO_PROVIDER);
		...
		return new GDE3(problem, population, PARETO_PROVIDER, selection, variation, initialization);
	}
	...
	private Algorithm neweNSGAII(TypedProperties properties, Problem problem) {
		...
		NondominatedSortingPopulation population = new NondominatedSortingPopulation(dominanceComparatorProvider);
	...
	private Algorithm newRandomSearch(TypedProperties properties, 
		...
		if (properties.contains("epsilon")) {
			...
			archive = new NondominatedPopulation(PARETO_PROVIDER);
		}
