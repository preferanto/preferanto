...
import org.moeaframework.core.PreferenceProblem;
...
class ProblemBuilder {
	...
	private DominanceComparatorProvider preferenceComparatorProvider;
	...
	ProblemBuilder withPreferenceComparatorProvider(DominanceComparatorProvider preferenceComparatorProvider) {
		this.preferenceComparatorProvider = preferenceComparatorProvider;
		
		return this;
	}

	public DominanceComparatorProvider getPreferenceComparatorProvider() {
		return preferenceComparatorProvider;
	}
	...
	NondominatedPopulation newArchive() {
		if (epsilon == null) {
			return new NondominatedPopulation(PARETO_PROVIDER);
		} else {
	...
	Problem getProblemInstance() {
		...
		if (problemClass != null) {
			try {
				problem = (Problem)ConstructorUtils.invokeConstructor(problemClass, problemArguments);
		...
		if(preferenceComparatorProvider != null) {
			problem = new PreferenceProblem(problem, preferenceComparatorProvider);
		}
		return problem;
	}
