...
public class ChainedComparator implements DominanceComparator, Serializable {
	...
	public int compare(Solution solution1, Solution solution2) {
		for (DominanceComparator comparator : comparators) {
			...
			if (flag != 0) {
				...
				currentComparator = (comparator instanceof ChainedComparator)
						? ((ChainedComparator)comparator).getCurrentComparator()
						: comparator.getClass().getSimpleName();
				Integer counter = counters.get(currentComparator);
				if(counter == null) {
					counter = 0;
				}
				counter++;
				counters.put(currentComparator, counter);
				return flag;
			}
		}
	...
	public String getCurrentComparator() {
