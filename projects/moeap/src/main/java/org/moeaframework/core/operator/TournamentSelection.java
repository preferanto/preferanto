...
//	}
	 ...
	 */
	public TournamentSelection(DominanceComparatorProvider comparatorProvider) {
		this(2, comparatorProvider);
	}
...
//	}
	 ...
	 * @param comparator the comparator used to determine the tournament winner
	 */
	public TournamentSelection(int size, DominanceComparatorProvider comparatorProvider) {
		super();
		this.size = size;
	...
	private DominanceComparator lastComparator = null;
	...
	private Solution select(Population population) {
		...
		if((lastComparator != null) && (population == lastPopulation) && !population.isModified()) {
			comparator = lastComparator;
		} else {
			comparator = comparatorProvider.getComparator(population);
			population.setModified(false);
